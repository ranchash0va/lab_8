...
class DataHandler<T>
{
    public void SaveToBinaryFile(T data, string fileName)
    {
        try
        {
            ...
            Console.WriteLine($"Данные успешно сохранены в бинарном файле '{fileName}'.");
        }
        catch (Exception ex)
        {
    ...
    public void SaveToJsonFile(T data, string fileName)
    {
        try
        {
            ...
            {
                UseSimpleDictionaryFormat = true, // для красивого форматирования JSON
            ...
            Console.WriteLine($"Данные успешно сохранены в JSON файле '{fileName}'.");
        }
        catch (Exception ex)
        {
